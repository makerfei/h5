...
function getGoodsDetail() {
  API_GOODS.goodsDetail({ id: route.query.id }).then((res) => {
    ...
    wxShare({
      ...
      link:`${location.protocol}//`+ (import.meta.env.PROD ? import.meta.env.VITE_APP_API_HOST : location.host)+`/api/shortlink/goods/${unref(basicInfo).id}`
    });
...
function onConcatService() {
  //Toast('未开放：客服');
  window.open(`${location.protocol}//`+ (import.meta.env.PROD ? import.meta.env.VITE_APP_API_HOST : location.host)+ '/api/shortlink/wx/kf')
}

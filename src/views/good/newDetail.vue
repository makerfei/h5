...
export default {
    ...
    data() {
        return {
            ...
            videoList: [{
                cover: 'http://clips.vorwaerts-gmbh.de/big_buck_bunny.mp4',
                url: 'https://www.mgdg.shop/img/2023_04_09/1680986997434.jpg'
            },
            {
                cover: 'https://www.mgdg.shop/img/2023_04_09/1680986986735.mp4',
                url: 'https://www.mgdg.shop/img/2023_04_09/1680986997434.jpg'
            }
            ],
            isVideoShow: true,
    ...
    methods: {
        ...
        playvideo(event) {
            let video = document.querySelectorAll('video')[this.current*2];
            console.log("playvideo：" + this.current);
            this.isVideoShow = false;
            this.iconPlayShow = false
            this.iconPauseShow = true
            video.play();
            document.querySelectorAll('video')[this.current*2+1].play();
            this.displayBtn()
        },
...
<template>
    <div class="small-videl-detail">
                ...
                <van-swipe-item v-for="(item, index) in videoList" :key="index" class="product_swiper">
                    <div class="video_container">
                        <!--video属性
                                          ...
                                          -->
                                          <!-- 封面 -->
                        <!-- <img  class="play"  @click="playvideo" :src="item.url" /> -->


                        <video class="play" width="100%" height="100%" webkit-playsinline="true" x5-playsinline=""
                            playsinline preload="auto" :poster="item.url" :src="item.cover" :playOrPause="playOrPause"
                            x-webkit-airplay="allow" x5-video-orientation="portrait" @click="pauseVideo"
                            @ended="onPlayerEnded($event)" loop="loop">
                        </video>
                        ...
                        <!-- 播放暂停按钮 -->
                        <van-icon name="pause-circle-o" color="white" v-show="iconPauseShow" size="90" class="icon_play"
                            @click="pauseVideo" />
                        <van-icon name="play-circle-o" color="white" v-show="iconPlayShow" size="90" class="icon_play"
                            @click="playvideo" />
                    </div>
                </van-swipe-item>
            </van-swipe>
                            ...
                            <van-cell title="徐女士">
                                ...
                                <template #icon>
                                    <div class="" style="margin-right: .23rem">
                                        <van-image width="2rem" height="2rem" fit="fill" round
                                ...
                                <template #label>
                                    <div class="ping_lun_info">
                                        ...
                                        <div class="ping_lun_other">
                                            ...
                                            <van-icon name="good-job-o" class="pinglun_label" size="1.2rem" badge="99+"
                                                color="#ee0a24" />
                                            <van-tag plain class="pinglun_label" round color="#7232dd"
                                                @click="detailShow = true">评论 12</van-tag>
                                        </div>
                                    </div>
                                </template>
                            </van-cell>
            ...
            <div class="content">
                <van-form @submit="onSubmit">
                    <van-cell-group inset>
                        <van-field v-model="message" rows="8" size="large" type="textarea" maxlength="100"
                            placeholder="您的评论..." show-word-limit />
                    </van-cell-group>
        ...
        <!-- 分享面板 -->
        <van-share-sheet v-model:show="share" title="立即分享给好友" :options="options" @select="onSelect" />
    </div>
</template>
